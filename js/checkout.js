...
function showProductCart() {
    ...
    currentuser.cart.forEach(item => {
        ...
        listOrderHtml += `<div class="products-total">
        <div class="count">${product.soluong}x</div>
        <div class="info-products">
            <div class="name-products">${product.title}</div>
        </div>
    </div>`
    })
...
function showProductBuyNow(product) {
    let listOrder = document.getElementById("list-order-checkout");
    let listOrderHtml = `<div class="products-total">
        <div class="count">${product.soluong}x</div>
        <div class="info-products">
<div class="name-products">${product.title}</div>
        </div>
...
function dathangngay() {
    ...
    datHangNgayBtn.onclick = () => {
        if(localStorage.getItem('currentuser')) {
            let productId = parseInt(datHangNgayBtn.getAttribute("data-product"));
            ...
            // Check kho lần nữa
            if(soluong > a.quantity) {
                toast({ title: 'Hết hàng', message: 'Sản phẩm không đủ số lượng tồn kho!', type: 'error', duration: 3000 });
                return;
            }
...
function xulyDathang(product) {
    ...
    if(tudenlay.classList.contains("active")){
        ...
        if(chinhanh1.checked) {
            diachinhan = "34 Lê Lợi - Thành phố Huế";
        }
        if(chinhanh2.checked) {
            diachinhan = "22 Đội Cung - Thành phố Huế";
        }
...
thoigiangiao = document.querySelector(".choise-time").value;
    ...
    let products = JSON.parse(localStorage.getItem("products")); // Lấy danh sách sản phẩm để trừ kho
    
    let madon = createId(order);
    let tongtien = 0;
    
    // Xác định danh sách hàng cần mua
    let itemsToBuy = [];
    if(product == undefined) {
        itemsToBuy = currentUser.cart;
    } else {
        itemsToBuy.push(product);
    }

    // Duyệt qua từng sản phẩm để tính tiền và TRỪ KHO
    itemsToBuy.forEach(item => {
        ...
        // Trừ tồn kho
        let productInStore = products.find(p => p.id == item.id);
        if (productInStore) {
            productInStore.quantity = productInStore.quantity - item.soluong;
        }
    });
    ...
    if(tennguoinhan == "" || sdtnhan == "" || diachinhan == "") {
        ...
        localStorage.setItem("products", JSON.stringify(products)); // Cập nhật lại kho
